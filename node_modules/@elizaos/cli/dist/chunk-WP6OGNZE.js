...
async function getPackageInfo() {
  ...
  try {
    const fileContent = await import("fs/promises").then(
      (fs12) => fs12.readFile(packageJsonPath, "utf8")
    );
...
async function getGitHubCredentials() {
  ...
  const { getGitHubToken: getGitHubToken2 } = await import("./registry-MJQMACYE.js");
...
// src/utils/character-parser.ts
function parseCharacterPaths(characterPaths) {
  if (!characterPaths) {
    return [];
  }
  ...
  for (const path19 of paths) {
    const cleanPath = path19.replace(/^["']|["']$/g, "");
  ...
  try {
    ...
    if (opts.output !== void 0) {
      if (retrievedAgents.length === 1) {
        const agent2 = retrievedAgents[0];
        ...
        for (const agent2 of retrievedAgents) {
          const { id, createdAt, updatedAt, enabled, ...agentConfig } = agent2;
          const filename = `${agent2.name || agent2.id || "agent"}.json`;
          const jsonPath = path11.resolve(process.cwd(), filename);
          writeFileSync(jsonPath, JSON.stringify(agentConfig, null, 2));
          console.log(`Saved agent configuration to ${jsonPath}`);
        }
    ...
    for (const agent2 of retrievedAgents) {
      console.log("");
      displayAgent(agent2, `Agent Details: ${agent2.name || agent2.id}`);
      if (opts.json) {
        const { id, createdAt, updatedAt, enabled, ...agentConfig } = agent2;
...
// src/utils/character-finder.ts
import * as fs9 from "fs";
import * as path12 from "path";
import { logger as logger11 } from "@elizaos/core";
import { globbySync } from "globby";
function resolveCharacterPath(characterPath) {
  if (path12.isAbsolute(characterPath) && fs9.existsSync(characterPath)) {
  ...
  const baseDirs = [
    process.cwd(),
    path12.join(process.cwd(), "characters"),
    path12.join(process.cwd(), "agents"),
    path12.join(process.cwd(), "src", "characters"),
    path12.join(process.cwd(), "src", "agents")
  ];
  for (const baseDir of baseDirs) {
    for (const pathToTry of pathsToTry) {
      const fullPath = path12.join(baseDir, pathToTry);
  ...
  const fileName = path12.basename(characterPath);
  for (const file of allCharacterFiles) {
    const baseName = path12.basename(file, path12.extname(file));
    const fileNameWithoutExt = fileName.replace(/\.(json|ts)$/, "");
    if (baseName === fileNameWithoutExt || path12.basename(file) === fileName) {
      return file;
    }
  }
  return null;
}
function findAllCharacterFiles(directory) {
  try {
    ...
    logger11.debug(`Error scanning directory ${directory}: ${error}`);
    return [];
  }
}
function isValidCharacterStructure(data) {
  ...
  const hasRequiredField = data.bio || data.system || Array.isArray(data.messageExamples) && data.messageExamples.length > 0;
  return hasRequiredField;
}
function hasCharacterExport(filePath) {
  try {
    if (filePath.includes("__tests__") || filePath.includes(".test.") || filePath.includes(".spec.") || filePath.includes("/test/") || filePath.includes("/tests/")) {
      return false;
    }
    const content = fs9.readFileSync(filePath, "utf-8");
    if (content.includes("vitest") || content.includes("jest") || content.includes("@testing-library") || content.includes("describe(") || content.includes("it(") || content.includes("test(")) {
      return false;
    }
    const patterns = [
    ...
    if (hasCharacterPattern) {
      ...
      const hasBio = /bio\s*:\s*["'`][\s\S]+["'`]/.test(content) || /bio\s*:\s*\[[\s\S]*?\]/.test(content);
...
async function isValidCharacterFile(filePath) {
...
async function findCharactersFromPaths(characterPaths) {
  const characterMap = /* @__PURE__ */ new Map();
  for (const characterPath of characterPaths) {
    try {
      const resolvedPath = resolveCharacterPath(characterPath);
      if (!resolvedPath) {
        logger11.error(`Character file not found: ${characterPath}`);
        continue;
      }
      const isValid = await isValidCharacterFile(resolvedPath);
      if (!isValid) {
        logger11.error(`Invalid character file: ${characterPath}`);
        continue;
      }
      const baseName = path12.basename(resolvedPath, path12.extname(resolvedPath));
      characterMap.set(baseName, resolvedPath);
    } catch (error) {
      logger11.error(`Failed to process character from ${characterPath}: ${error}`);
    }
  }
  return characterMap;
}
async function loadCharacterQuietly(filePath) {
  const content = fs9.readFileSync(filePath, "utf-8");
  return JSON.parse(content);
}
function getJsonCharacterName(filePath) {
  try {
    const content = fs9.readFileSync(filePath, "utf-8");
...
function getTsCharacterName(filePath) {
  try {
    const content = fs9.readFileSync(filePath, "utf-8");
    const patterns = [
...
async function getCharacterInfo(filePath) {
  ...
  if (!name) {
    name = path12.basename(filePath, path12.extname(filePath));
  }
...
agent.command("get").alias("g").description("Get agent details").requiredOption(
  "-c, --character <characters...>",
  "agent character name(s), id(s), or index number(s) from list"
).option("-j, --json", "display agent configuration as JSON in the console").option("-o, --output [file]", "save agent config to JSON (defaults to {name}.json)").option("-r, --remote-url <url>", "URL of the remote agent runtime").option("-p, --port <port>", "Port to listen on", (val) => Number.parseInt(val)).action(getAgent);
agent.command("start").alias("s").description("Start agents with character profiles").option(
  "-c, --character <characters...>",
  "Character name(s), file path(s), or existing agent name(s)"
).option("--remote-character <url>", "URL to remote character JSON file").option("-r, --remote-url <url>", "URL of the remote agent runtime").option("-p, --port <port>", "Port to listen on", (val) => Number.parseInt(val)).addHelpText(
  "after",
  `
Examples:
...
agent.command("stop").alias("st").description("Stop agents").option(
  "-c, --character <characters...>",
  "agent character name(s), id(s), or index number(s) from list"
).option("--all", "stop all running agents").option("-r, --remote-url <url>", "URL of the remote agent runtime").option("-p, --port <port>", "Port to listen on", (val) => Number.parseInt(val)).action(stopAgent);
agent.command("remove").alias("rm").description("Remove agents").requiredOption(
  "-c, --character <characters...>",
  "agent character name(s), id(s), or index number(s) from list"
).option("-r, --remote-url <url>", "URL of the remote agent runtime").option("-p, --port <port>", "Port to listen on", (val) => Number.parseInt(val)).action(removeAgent);
agent.command("set").description("Update agent configuration").requiredOption(
  "-c, --character <character>",
  "agent character name, id, or index number from list"
).option("--config <json>", "agent configuration as JSON string").option("-f, --file <path>", "path to agent configuration JSON file").option("-r, --remote-url <url>", "URL of the remote agent runtime").option("-p, --port <port>", "Port to listen on", (val) => Number.parseInt(val)).action(setAgentConfig);
agent.command("clear-memories").alias("clear").description("Clear all memories for agents").requiredOption(
  "-c, --character <characters...>",
  "agent character name(s), id(s), or index number(s) from list"
).option("-r, --remote-url <url>", "URL of the remote agent runtime").option("-p, --port <port>", "Port to listen on", (val) => Number.parseInt(val)).action(clearAgentMemories);
...
function detectPluginContext(pluginName) {
  ...
  let packageInfo;
  try {
    packageInfo = JSON.parse(readFileSync8(packageJsonPath, "utf-8"));
  } catch (error) {
...
async function readPackageJson(repository) {
  ...
  try {
    if (existsSync11(packageJsonPath)) {
      return JSON.parse(readFileSync9(packageJsonPath, "utf-8"));
    }
  } catch (error) {
...
async function tryImporting(importPath, strategy, repository) {
  try {
    ...
    logger15.debug(`[DEBUG] \u2705 Import succeeded: ${strategy} (${importPath})`);
    return module;
  } catch (error) {
    logger15.debug(
...
async function loadPluginModule(repository) {
  ...
  for (const strategy of strategies) {
    logger15.debug(`[DEBUG] Trying strategy: ${strategy.name} for '${repository}'`);
    const result = await strategy.tryImport(repository);
    if (result) {
      logger15.success(`[DEBUG] \u2705 Strategy '${strategy.name}' succeeded for '${repository}'`);
      return result;
    } else {
      logger15.debug(`[DEBUG] \u274C Strategy '${strategy.name}' failed for '${repository}'`);
    }
  }
...
`);
  ...
  async runE2eTests(options) {
    ...
    try {
      ...
      if (!fs10.existsSync(e2eDir)) {
        safeLogger.debug("No e2e directory found, skipping e2e tests");
        return;
      }
      safeLogger.info("\nRunning e2e tests...");
      const walk = (dir) => fs10.readdirSync(dir, { withFileTypes: true }).flatMap(
      ...
      const hasDistE2e = fs10.existsSync(distE2eDir);
...
import { existsSync as existsSync15, promises as fs11 } from "fs";
...
async function ensureElizaDir2() {
  try {
    await fs11.mkdir(ELIZA_DIR, { recursive: true });
  } catch (error) {
  }
}
async function getRegistrySettings() {
  ...
  try {
    const content = await fs11.readFile(REGISTRY_SETTINGS_FILE, "utf-8");
    return JSON.parse(content);
  } catch (error) {
    return {
...
async function saveRegistrySettings(settings) {
  await ensureElizaDir2();
  await fs11.writeFile(REGISTRY_SETTINGS_FILE, JSON.stringify(settings, null, 2));
}
async function getEnvVar(key) {
  try {
    const envContent = await fs11.readFile(ENV_FILE, "utf-8");
...
async function setEnvVar(key, value) {
  ...
  let envContent = "";
  try {
    envContent = await fs11.readFile(ENV_FILE, "utf-8");
  } catch (error) {
  }
  const env = dotenv3.parse(envContent);
  env[key] = value;
  const newContent = Object.entries(env).map(([k, v]) => `${k}=${v}`).join("\n");
  await fs11.writeFile(ENV_FILE, newContent);
}
async function getGitHubToken() {
  try {
    ...
    if (existsSync15(globalEnvPath) && globalEnvPath !== envPath) {
      const envContent = await fs11.readFile(globalEnvPath, "utf-8");
      const env = dotenv3.parse(envContent);
      return env.GITHUB_TOKEN;
    }
  } catch (error) {
...
async function saveRegistryCache(registry) {
  try {
    await ensureElizaDir2();
    await fs11.writeFile(REGISTRY_CACHE_FILE, JSON.stringify(registry, null, 2));
...
async function getLocalRegistryIndex() {
  ...
  try {
    if (existsSync15(REGISTRY_CACHE_FILE)) {
      const cacheContent = await fs11.readFile(REGISTRY_CACHE_FILE, "utf-8");
...
async function checkDataDir() {
  ...
  try {
    const settingsContent = await fs11.readFile(REGISTRY_SETTINGS_FILE, "utf-8");
...
async function initializeDataDir() {
  await ensureElizaDir2();
  try {
    await fs11.access(ENV_FILE);
  } catch {
    await fs11.writeFile(ENV_FILE, "");
  }
  try {
    await fs11.access(REGISTRY_SETTINGS_FILE);
  } catch {
