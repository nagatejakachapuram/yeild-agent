import { OptionValues } from 'commander';

/**
 * Get command implementation - retrieves and displays agent details
 */
declare function getAgent(opts: OptionValues): Promise<void>;
/**
 * Remove command implementation - deletes agents
 */
declare function removeAgent(opts: OptionValues): Promise<void>;
/**
 * Clear memories command implementation - clears all memories for agents
 */
declare function clearAgentMemories(opts: OptionValues): Promise<void>;
/**
 * Set command implementation - updates agent configuration
 */
declare function setAgentConfig(opts: OptionValues): Promise<void>;

/**
 * Start command implementation - starts an agent with character configuration
 */
declare function startAgent(options: OptionValues): Promise<void>;
/**
 * Stop command implementation - stops a running agent
 */
declare function stopAgent(opts: OptionValues): Promise<void>;

export { clearAgentMemories, getAgent, removeAgent, setAgentConfig, startAgent, stopAgent };
