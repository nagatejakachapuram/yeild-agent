...
var promptForEnvVar = async (varName, description, type) => {
...
${emoji.rocket(`Plugin ${packageName} requires environment variables:`)}`);
  ...
  for (const [varName, config2] of Object.entries(envRequirements)) {
    const { type = "string", description = "No description available" } = config2;
...
// src/commands/plugins/utils/character-updater.ts
import { logger as logger3 } from "@elizaos/core";
import * as fs2 from "fs";
import * as path9 from "path";
async function loadCharacterFile(characterPath) {
...
function addPluginToCharacter(character, pluginName) {
  const plugins2 = character.plugins || [];
  if (plugins2.includes(pluginName)) {
    logger3.info(`Plugin '${pluginName}' is already in character '${character.name}'`);
    return character;
  }
  return {
    ...character,
    plugins: [...plugins2, pluginName]
  };
}
function removePluginFromCharacter(character, pluginName) {
  const plugins2 = character.plugins || [];
  if (!plugins2.includes(pluginName)) {
    logger3.info(`Plugin '${pluginName}' is not in character '${character.name}'`);
    return character;
  }
  return {
    ...character,
    plugins: plugins2.filter((p) => p !== pluginName)
  };
}
async function updateJsonCharacterFile(filePath, character) {
  const content = JSON.stringify(character, null, 2);
  fs2.writeFileSync(filePath, content, "utf-8");
  logger3.info(`Updated character file: ${filePath}`);
}
async function updateCharacterFile(characterFile, pluginName, operation) {
  const updatedCharacter = operation === "add" ? addPluginToCharacter(characterFile.character, pluginName) : removePluginFromCharacter(characterFile.character, pluginName);
...
async function updateCharacterFiles(pluginName, opts) {
  if (!opts.character) {
    logger4.error(
  ...
  const characterPaths = parseCharacterPaths(opts.character);
  ...
  for (const characterPath of characterPaths) {
    try {
      ...
      logger4.info(`\u2705 Added plugin '${pluginName}' to character '${characterFile.character.name}'`);
    } catch (error) {
      logger4.error(`Failed to update character file ${characterPath}:`, error);
      logger4.warn("Plugin was installed but character file update failed.");
      logger4.info("You can manually add the plugin to your character file.");
      hasFailures = true;
    }
  }
  if (hasFailures) {
    logger4.warn("Some character files could not be updated. Plugin installation was successful.");
    logger4.info(
...
async function addPlugin(pluginArg, opts) {
  ...
  if (!opts.character || Array.isArray(opts.character) && opts.character.length === 0) {
    logger4.error("No character files specified.");
    logger4.info("Use --character to specify one or more character files to update.");
    logger4.info("Example: elizaos plugins add openrouter --character ./characters/my-agent.json");
    process.exit(1);
  }
  const cwd = process.cwd();
  const directoryInfo = detectDirectoryType(cwd);
  ...
  if (installedPluginName) {
    ...
    logger4.info(`Updating character files...`);
...
async function updateCharacterFilesForRemoval(pluginName, opts) {
  if (!opts.character) {
    logger5.error(
      "No character files specified. Use --character to specify character files to update."
    );
  ...
  for (const characterPath of characterPaths) {
    try {
      ...
      logger5.error(`Failed to update character file ${characterPath}:`, error);
      process.exit(1);
    }
  }
}
async function removePlugin(plugin, opts) {
  if (!opts.character || Array.isArray(opts.character) && opts.character.length === 0) {
    logger5.error("No character files specified.");
    logger5.info("Use --character to specify one or more character files to update.");
    logger5.info(
  ...
  if (!packageNameToRemove) {
    ...
    logger5.info("The plugin may have already been removed from package.json.");
    logger5.info("Attempting to remove from character files anyway...");
    await updateCharacterFilesForRemoval(plugin, opts);
    process.exit(0);
  }
  logger5.info(`Removing plugin from character files...`);
...
// src/commands/plugins/actions/installed-plugins.ts
import { logger as logger7 } from "@elizaos/core";
async function showInstalledPlugins(characterPaths) {
  const cwd = process.cwd();
  const directoryInfo = detectDirectoryType(cwd);
  if (!directoryInfo || !directoryInfo.hasPackageJson) {
    console.error(
  ...
  if (parsedPaths.length > 0) {
    ...
    const characterPluginMap2 = /* @__PURE__ */ new Map();
    for (const [name, filePath] of characterFilePaths) {
      try {
        ...
        const plugins2 = character.plugins || [];
        characterPluginMap2.set(characterName, plugins2);
      } catch (error) {
        logger7.error(`Failed to load character from ${filePath}: ${error}`);
      }
    }
    logHeader("Character Plugin Mapping:");
    for (const [characterName, plugins2] of characterPluginMap2) {
      console.log(`
${characterName}:`);
      if (plugins2.length === 0) {
        console.log("  (no plugins)");
      } else {
        plugins2.forEach((plugin) => console.log(`  - ${plugin}`));
      }
    }
    return;
  }
  const allCharacterFiles = findAllCharacterFiles(cwd);
  const characterPluginMap = /* @__PURE__ */ new Map();
  for (const filePath of allCharacterFiles) {
    try {
      ...
      const plugins2 = character.plugins || [];
      characterPluginMap.set(characterName, plugins2);
    } catch (error) {
    }
  }
  ...
  for (const [characterName, plugins2] of characterPluginMap) {
...
The upgraded plugin has been copied to your current directory.`);
      console.log(`
Next steps:`);
      console.log(`1. cd ${path11.basename(result.repoPath ?? "")}`);
      ...
      logger8.error(`Plugin upgrade failed: ${result.error?.message}`);
      process.exit(1);
    }
  } catch (error) {
    handleError(error);
...
plugins.command("add").alias("install").description("Add a plugin to a character").argument("<plugin>", 'plugin name (e.g., "openrouter", "plugin-abc", "@elizaos/plugin-abc")').requiredOption("-c, --character <paths...>", "Character file(s) to update").option("-s, --skip-env-prompt", "Skip prompting for environment variables").option("--skip-verification", "Skip plugin import verification after installation").option("-b, --branch <branchName>", "Branch to install from when using monorepo source", "main").option("-T, --tag <tagname>", "Specify a tag to install (e.g., beta)").action(async (pluginArg, opts) => {
  try {
...
plugins.command("installed-plugins").description("List plugins found in the project dependencies").option("-c, --character <paths...>", "Show plugins for specific character file(s)").action(async (opts) => {
  try {
...
plugins.command("remove").aliases(["delete", "del", "rm"]).description("Remove a plugin from a character").argument("<plugin>", 'plugin name (e.g., "openrouter", "plugin-abc", "@elizaos/plugin-abc")').requiredOption("-c, --character <paths...>", "Character file(s) to update").action(async (plugin, opts) => {
  try {
...
async function updateRegistryIndex(packageMetadata, dryRun = false) {
  try {
    ...
    try {
      await fs4.access(path14.dirname(indexPath));
    } catch {
      await fs4.mkdir(path14.dirname(indexPath), { recursive: true });
      try {
...
async function getNpmUsername() {
  ...
  try {
    ...
    const useExisting = await clack5.confirm({
      message: `Use NPM account "${currentUser}" for package naming?`,
      initialValue: true
    });
    if (clack5.isCancel(useExisting)) {
...
async function updateDependencies(cwd, isPlugin2, options = {}) {
  ...
  if (hasMajorUpdates) {
    const confirmMajor = await clack6.confirm({
      message: "This update includes major version changes. Continue?",
      initialValue: false
    });
    if (clack6.isCancel(confirmMajor)) {
...
async function checkCliVersion() {
  try {
    const cliPackageJsonPath = path17.resolve(
      path17.dirname(fileURLToPath(import.meta.url)),
      "../package.json"
    );
    ...
    if (latestVersion && latestVersion !== currentVersion) {
      ...
      const update2 = await clack7.confirm({
        message: "Update CLI before publishing?",
        initialValue: false
      });
      if (clack7.isCancel(update2)) {
...
var publish = new Command5().name("publish").description("Publish a plugin to npm, GitHub, and the registry").option("--npm", "publish to npm only (skip GitHub and registry)", false).option("-t, --test", "test publish process without making changes", false).option("-d, --dry-run", "generate registry files locally without publishing", false).option("--skip-registry", "skip publishing to the registry", false).hook("preAction", async () => {
  ...
  try {
    ...
    if (detectedType === "plugin" && !packageJson.name.includes("plugin-")) {
      ...
      const proceed = await clack8.confirm({
        message: "Proceed anyway?",
        initialValue: false
      });
      if (clack8.isCancel(proceed)) {
...
async function loadProject(dir) {
  try {
    ...
    const packageJson = JSON.parse(fs10.readFileSync(path21.join(dir, "package.json"), "utf8"));
  ...
  try {
    ...
    if (options.character && options.character.length > 0) {
      ...
      for (const charPath of characterPaths) {
        ...
        if (!resolvedPath) {
          logger19.error(`Character file not found: ${charPath}`);
...
${errors.join("\n")}`);
      ...
      logger19.info("No character files specified. Searching for available characters...");
      ...
      for (const file of allCharacterFiles) {
        ...
        if (characterInfo) {
          const relativePath = path22.relative(cwd, characterInfo.path);
      ...
      if (validCharacters.length > 0) {
        logger19.info(`Found ${validCharacters.length} character file(s) in project:`);
        for (const char of validCharacters) {
          logger19.info(`  - ${char.name} (${char.path})`);
        }
        logger19.info("\nTo use a specific character, run:");
        logger19.info("  elizaos start --character <character-name>");
      }
      let projectLoaded = false;
      try {
        if (dirInfo.hasPackageJson && dirInfo.type !== "non-elizaos-dir") {
          logger19.debug("Checking for project configuration...");
...
Running in plugin test mode with default Eliza character.`);
              logger19.info(`Testing plugin: ${project.pluginModule?.name || "unknown"}`);
            } else {
              logger19.info(
                `
Running ${project.agents.length} agent(s) from project configuration:`
              ...
              for (const agent2 of project.agents) {
                if (agent2.character) {
                  logger19.info(`  - ${agent2.character.name}`);
                }
              }
            }
          }
...
async function installPluginDependencies(projectInfo) {
  ...
  if (project.isPlugin && project.pluginModule?.dependencies && project.pluginModule.dependencies.length > 0) {
    ...
    const { installPlugin: installPlugin2 } = await import("./utils-JWYJVLQ4.js");
...
import { AgentServer as AgentServer2, jsonToCharacter as jsonToCharacter2, loadCharacterTryPath as loadCharacterTryPath2 } from "@elizaos/server";
...
async function runE2eTests(testPath, options, projectInfo) {
  ...
  try {
    ...
    try {
      logger23.info("Configuring database...");
      await promptForEnvVars("pglite");
      logger23.info("Database configuration completed");
    } catch (error) {
      logger23.error("Error configuring database:", error);
    ...
    try {
      ...
      const targetPath = testPath ? path26.resolve(monorepoRoot, testPath) : process.cwd();
      ...
      server.loadCharacterTryPath = loadCharacterTryPath2;
...
import path28, { dirname } from "path";
...
async function main() {
  ...
  const packageJsonPath = path28.resolve(__dirname, "../package.json");
  let version = "0.0.0";
